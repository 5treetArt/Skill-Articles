...
fun Date.humanizeDiff(date: Date = Date()): String {
    ...
    return when(Math.abs(diff)) {
        ...
        in (1 * SECOND)..(360 * DAY) -> String.format(getFormat(diff), toUnitsString(Math.abs(diff)))
...
enum class TimeUnits{
    ...
    private fun getFromMillis(millis: Long): Long = Math.round(millis.toDouble() / getInMillis())
